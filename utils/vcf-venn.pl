...
$opt{project} ||="GermEval";
my $vcftool="/hpcf/apps/vcftools/install/0.1.12b/bin";
my $tabix="/hpcf/apps/htslib/install/1.2.1/bin";
my @vcf=readlist($opt{vcf});

my @vcfgz;
for(my $i=0;$i<@vcf;$i++){
   my $title=$1 if ($vcf[$i]=~/(.*)\.vcf\.gz$/);
   `$tabix/tabix -p vcf $title.vcf.gz` unless (-e "$title.vcf.gz.tbi");
   push @vcfgz, "$title.vcf.gz";
}
my $vcfcom=join(" ",@vcfgz);
print ">>> CMD: $vcftool/vcf-compare $vcfcom\n";
`$vcftool/vcf-compare $vcfcom > vcfvenn.$opt{project}.raw`;
